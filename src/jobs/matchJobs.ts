    ...
    for (const user of users) {
      try {
        ...
        if (user.preferences?.matchingEnabled === false) {
          console.log(
            `Skipping user ${user.email} - matching disabled by user`
          );
          continue;
        }
          ...
          if (!alreadyChargedToday) {
            ...
            if (updatedUser) {
              ...
              console.log(
                `Deducted $0.30 from ${
                  user.email
                }'s wallet. New balance: $${updatedUser.walletBalance.toFixed(
                  2
                )}`
              );
            }
        ...
        // Log error but continue to next user - don't let one user's failure stop the whole job
        console.error(
          `[ERROR] Failed to process user ${user.email}, continuing to next user:`,
          userError
        );
      }
    }
