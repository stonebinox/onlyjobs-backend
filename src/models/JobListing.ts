...
// Invariant enforcement: dedupKey === computeDedupKey(url) for every .save()/.create().
// Always overrides any caller-supplied dedupKey — a wrong non-empty key would bypass the unique index.
// insertMany/bulkWrite bypass hooks (confirmed: no such paths exist).
