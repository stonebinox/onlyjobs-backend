...
export const sendPasswordResetEmail = async (
  ...
  if (!ensureConfigured()) {
    console.error(`[EMAIL] Skipping password reset - Resend not configured`);
    return false;
  }

  if (!email) {
    console.error("[EMAIL] Skipping password reset - email missing");
    return false;
  }
  ...
  const subject = "Reset your password for onlyjobs";
  const html = `
    <div style="font-family: Arial, sans-serif; color: #1f2937;">
      <h2 style="color:#111827;">Reset your password</h2>
      <p>We received a request to reset your password for your onlyjobs account.</p>
      <p>Click the button below to set a new password. If you didn't request this, you can safely ignore this email.</p>
      <div style="margin-top:20px; text-align:center;">
        <a href="${resetUrl}" style="display:inline-block; background-color:#111827; color:#ffffff; padding:12px 20px; text-decoration:none; border-radius:6px; font-weight:600;">Reset Password</a>
      </div>
      <p style="margin-top:16px; color:#6b7280; font-size:14px;">This link will expire in 1 hour and can be used only once.</p>
      <p style="margin-top:16px;">– The onlyjobs team</p>
    </div>
  `;
  ...
  try {
    ...
    console.log(`[EMAIL] Sending password reset email to ${email} from ${FROM_EMAIL}`);
    const result = await resendClient.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject,
    ...
    console.log(
      `[EMAIL] ✓ Sent password reset email to ${email} (ID: ${
        result.data?.id || "unknown"
      })`
    );
    return true;
